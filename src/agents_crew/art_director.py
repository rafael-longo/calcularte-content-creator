...
First, analyze the brand context, post concept and caption. Based on the narrative, create a high-level storyboard.

If the `suggested_format` in the `post_idea` is a **Carousel**, your storyboard MUST follow this specific structure:
1.  **Slide 1 (Cover):** A compelling cover image that introduces the topic from the `post_idea.title`. It must grab attention and set the theme.
2.  **Slides 2 to N-2 (Content):** These are the core story slides. Break down the main narrative from the `caption` into a series of digestible, step-by-step visual points. Each slide should build on the last.
3.  **Slide N-1 (Connection - Optional but Recommended):** If the caption makes a connection to a Calcularte feature, create a dedicated slide for it. This slide should visually bridge the gap between the user's problem (discussed in the content slides) and the app's solution. If there's no explicit connection, you can omit this slide.
4.  **Slide N (CTA):** The final slide must be the standard Call-to-Action, which should always be  for saving the post, sharing, comenting and click the link in the bio (calcularte.com.br) to start using Calcularte.

This storyboard should be a simple, bulleted list outlining the concept for each slide, where each slide should be conceived and designed as an image with an overlaid text in the form fo a title and subtitle, and all slide images together should be a simplified, didactic, and step-by-step way to summarize the same content as the caption. You must decide the optimal number of slides (from 1 to 20) to tell the story effectively. For a simple meme or objective simple message, 1 slide is enough. For a step-by-step guide, more slides are needed. The final slide should always be a CTA (Call-to-action) image for saving the post, sharing, comenting and click the link in the bio (calcularte.com.br) to start using Calcularte (example CTA final image prompt: A clean, modern, and highly engaging call-to-action graphic. The background is a solid, elegant teal green. At the top, a large, empowering title in a bold, friendly white font says: "Planeje com Confiança!". Below this title, create three distinct sections in a horizontal layout, each featuring a large pastel pink icon with white text below it. On the left, a bookmark icon with "SALVE este guia". In the middle, a paper airplane icon with "COMPARTILHE com uma amiga". On the right, a speech bubble icon with "COMENTE sua ideia favorita". At the very bottom, centered and in a smaller font, add the text "Comece a se organizar em calcularte.com.br (link na bio!)".). Be creative with the flow.
